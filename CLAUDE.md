...
After filling the OTP field, `fillAndSubmit`/`fillAndSubmitWithAccount` normally auto-click the form's submit button ~600ms later. Sites running bot/automation detection across their auth flow (confirmed on Rippling, which runs Cloudflare) treat that synthetic click's `isTrusted: false` as suspicious and silently reject the request — the field shows the right code, but the POST fails as if the code were wrong, while the exact same value submitted via a real user click/Enter succeeds. There's no way to fake a trusted event from a content script (that would require `chrome.debugger`/CDP, a much bigger permission ask), so instead of a hardcoded site list this is detected automatically at runtime: `watchAutoSubmit()` checks ~3.5s after the auto-click whether it looks like it worked (page navigated, or `findOTPInput()` no longer finds an OTP field anywhere on the page) — if not, `markNoAutoSubmitHost()` persists the hostname in `chrome.storage.local` (`noAutoSubmitHosts`) and future fills on that host skip the auto-click, leaving the final submit to the user. No maintainer action needed per affected site.
