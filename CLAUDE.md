...
### Auto-submit self-healing (`extension/content.js`)
After filling the OTP field, `fillAndSubmit`/`fillAndSubmitWithAccount` normally auto-click the form's submit button ~600ms later. Sites running bot/automation detection across their auth flow (confirmed on Rippling, which runs Cloudflare) treat that synthetic click's `isTrusted: false` as suspicious and silently reject the request — the field shows the right code, but the POST fails as if the code were wrong, while the exact same value submitted via a real user click/Enter succeeds. There's no way to fake a trusted event from a content script (that would require `chrome.debugger`/CDP, a much bigger permission ask), so instead of a hardcoded site list this is detected automatically at runtime: `watchAutoSubmit()` checks ~5s after the auto-click whether it looks like it worked (page navigated, or `findOTPInput()` no longer finds an OTP field anywhere on the page). A single apparent failure only records a strike (`noAutoSubmitStrikes:<hostname>`) — a slow-but-successful flow (delayed redirect, async validation) can look identical to a rejected click for one check — and only the second strike (necessarily from a separate page load, since `_lastAutoSubmitAt` throttles to one attempt per minute) actually flags the host (`noAutoSubmit:<hostname>`) and skips future auto-clicks there, leaving the final submit to the user. No maintainer action needed per affected site. Each host's flag/strike count lives under its own storage key rather than a shared `{hostname: true}` object, so two tabs flagging different hosts around the same time can't race a read-modify-write and drop one of them.
